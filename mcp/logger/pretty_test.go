...
func TestPrettyWriterFormatsSafeOperationalEvents(t *testing.T) {
	...
	entries := []string{
		...
		`{"event":"upstream","request_id":"request-1","tool":"get_arrivals_for_stop","op":"stop","cache":"miss","params":"input=private","ms":12,"status":200,"bytes":512}`,
	...
	for _, expected := range []string{"[READY]", "[MISS]", "stop (get_arrivals_for_stop)", "[DRAIN]", "request_id=request-1"} {
