...
func TestToolResponseTypesMarshal(t *testing.T) {
	testCases := []struct {
		...
		{"arrival", ArrivalResponse{TripID: "unitrans_trip", RouteID: "unitrans_A", Predicted: true}},
		{"overview", StopOverviewResponse{StopID: "unitrans_1", Next: []ArrivalResponse{{TripID: "unitrans_trip"}}}},
		{"stop schedule", StopScheduleResponse{StopID: "unitrans_1", Routes: []RouteScheduleResponse{{RouteID: "unitrans_A"}}}},
		...
		{"route schedule", RouteScheduleStructureResponse{RouteID: "unitrans_A"}},
		{"shape", ShapeResponse{Points: "encoded"}},
		{"block", BlockResponse{ID: "unitrans_block"}},
		{"current time", CurrentTimeResponse{Time: "2026-01-01T00:00:00Z"}},
		{"metadata", MetadataResponse{RealtimeFeeds: map[string]string{"trip_updates": "2026-01-01T00:00:00Z"}}},
	}
