<script>
	...
	let { label = null, size: _size = 'md' } = $props();
...
<div class="flex flex-col items-center gap-3 py-4">
	...
	>
		...
		<div
			class="motion-streak motion-streak-delayed absolute left-12 top-5 h-px w-4 rounded-full bg-oba-200/60"
		></div>

		<!-- Road -->
		<div
			class="absolute bottom-2 left-0 right-0 h-2 bg-gradient-to-b from-zinc-200/70 to-transparent dark:from-zinc-700/60"
		></div>
		<div class="absolute bottom-1 left-0 right-0 h-px bg-zinc-300 dark:bg-zinc-600"></div>

		<!-- Dashed line -->
		<div
			class="road-dashes absolute bottom-[5px] left-0 h-px w-[400%]"
		...
		<div class="bus absolute bottom-1">
			<svg width="52" height="28" viewBox="0 0 52 28" fill="none">
				...
				<!-- Wheels -->
				<g class="wheel"
					><circle cx="12" cy="24" r="4.5" fill="#1c1c1e" /><circle
						cx="12"
						cy="24"
						r="2.5"
						fill="#333"
					...
					><circle cx="38" cy="24" r="4.5" fill="#1c1c1e" /><circle
						cx="38"
						cy="24"
						r="2.5"
						fill="#333"
		...
		<div
			class="stop-sign absolute right-5 top-2 flex h-3 w-5 items-center justify-center rounded-sm bg-oba-500 text-[7px] font-bold text-white shadow-sm"
		>
			+
		</div>
	</div>
...
<style>
	...
	@keyframes oba-headlight {
		...
		50% {
			opacity: 1;
			filter: drop-shadow(0 0 3px #fff9c4);
		}
	}
	@keyframes oba-stop-pulse {
		...
		100% {
			transform: scale(1);
			box-shadow: 0 0 0 0 rgb(76 175 80 / 0.35);
		}
		50% {
			transform: scale(1.08);
			box-shadow: 0 0 0 4px rgb(76 175 80 / 0);
		}
	}
