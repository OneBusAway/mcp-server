<script>
	...
	// eslint-disable-next-line svelte/valid-compile -- initial cache lookup only; provider changes trigger fetchLiveModels() which reassigns liveModels
	let liveModels = $state(settings.getCachedModels(provider));

	const MAP_STYLES = [
		{
			id: 'https://tiles.openfreemap.org/styles/bright',
			label: 'OpenFreeMap Bright — clean, minimal (recommended)',
		},
	...
	async function fetchLiveModels() {
		...
		try {
			...
			const all = (data.data ?? data.models ?? []).map((m) => ({
				id: m.id,
				label:
					(m.name ?? m.id).replace(/\s*\(free\)/i, '').trim() +
...
<div class="mx-auto max-w-[760px] px-5 py-10 sm:px-6">
	...
	>
				...
				<div>
					...
					{#if currentProvider.local}
						<p class="mt-1.5 text-xs text-zinc-400 dark:text-zinc-500">
							Searching <code class="rounded bg-zinc-100 px-1 dark:bg-zinc-800"
								>{currentProvider.baseUrl}/models</code
							> on this machine. Start the local server, then press refresh if needed.
						</p>
					{/if}
					{#if liveModels?.length === 0}
						<p class="mt-1 text-xs text-amber-500">
							No models found — check your API key or provider URL.
						</p>
					{/if}
				</div>
			</div>
		...
		>
			...
			<div class="space-y-4">
				<div>
					...
					{#if testResult}
						<p
							class="mt-1.5 flex items-center gap-1.5 text-xs {testResult === 'success'
								? 'text-oba-600 dark:text-oba-400'
								: 'text-red-500'}"
						>
							<Icon
								name={testResult === 'success' ? 'check' : 'alert-triangle'}
								cls="h-3.5 w-3.5 shrink-0"
							/>
		...
		>
			...
			<div class="space-y-2">
				...
				{/each}
				<div>
					<p class="mb-1 text-xs text-zinc-400 dark:text-zinc-500">
						Or paste a custom MapLibre style URL:
					</p>
					<input
						type="url"
						bind:value={mapStyle}
	...
	>
		...
		<ol class="space-y-2.5 text-sm text-zinc-600 dark:text-zinc-400">
			<li class="flex gap-3">
				<span
					class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-oba-500 text-xs font-bold text-white"
					>1</span
				>
				Start maglev:
				<code class="rounded bg-zinc-200 px-1.5 py-0.5 text-xs dark:bg-zinc-800"
					>cd maglev && make run</code
				>
			</li>
			<li class="flex gap-3">
				<span
				...
				Start oba-mcp:
				<code class="rounded bg-zinc-200 px-1.5 py-0.5 text-xs dark:bg-zinc-800"
					>cd oba-mcp && OBA_HTTP_AUTH_TOKEN=local-dev make serve-http</code
				>
			</li>
			<li class="flex gap-3">
				<span
					...
					>3</span
				>
				No API key? Pick <strong>Groq</strong> above — free account at
				<code class="rounded bg-zinc-200 px-1.5 py-0.5 text-xs dark:bg-zinc-800"
					>console.groq.com</code
				>
			</li>
			<li class="flex gap-3">
				<span
					...
					>4</span
				>
				Go to <a href="/chat" class="font-medium text-oba-600 underline dark:text-oba-400">Chat</a> and
				ask about buses!
			</li>
		</ol>
	</div>
